...
class MotionData(object):
    ...
    def extend(self, jv_list, ev_list=None, toggle_mesh=True):
        """
        :param jv_list:
        :param ev_list:
    ...
    def gripper_extend(self, ev_list, toggle_mesh=True):
        """
        :param jv_list:
        :param ev_list:
        :param mesh_list: auto gen if None, fill with None if [], assign other wise
        ...
        """
        # joint value list
        current_joint_values = self.robot.get_jnt_values()
        self._jv_list += [current_joint_values] * len(ev_list)
        # end effector value list
        self._ev_list += ev_list
        ...
        if toggle_mesh:
            ...
            tmp_mesh_list = []
            self.robot.backup_state()
            for i, ee_values in enumerate(ev_list):
                # if ev_list is None:
                self.robot.goto_given_conf(jnt_values=current_joint_values, ee_values=ee_values)
            ...
            self._mesh_list += [None] * len(ev_list)
            if len(self.robot.oiee_list) == 0:
                self._oiee_gl_pose_list += [None] * len(ev_list)
            else:
                oiee_gl_pose_list = []
                self.robot.backup_state()
                for i, jnt_values in enumerate(ev_list):
                    if ev_list is None:
                        self.robot.goto_given_conf(jnt_values=jnt_values)
                    else:
                        self.robot.goto_given_conf(jnt_values=jnt_values, ee_values=ev_list[i])
